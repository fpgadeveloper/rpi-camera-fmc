...
as the *target board* and is what `PetaLinux/Makefile` uses to select
...
`petalinux_only`, `baremetal_only`, or `both`. Most ZynqMP targets are
`petalinux_only`; `auboard` (MicroBlaze) and `genesyszu` are
`baremetal_only`. A `make bootimage TARGET=<t>` invocation at the top
level cascades:
...
make bootimage TARGET=t
  ...
  -> PetaLinux side (if applicable):
       PetaLinux/Makefile petalinux TARGET=t
         ...
         -> copy bsp/<target>/project-spec/* into the project    (optional, if it exists)
...
Edit `Vitis/common/src/*.c` (or `.h`) directly. The next `make -C
Vitis bootfile TARGET=<t>` rebuilds the application against the
existing platform; if you've changed the hardware (XSA) you'll need a
fresh workspace (`make -C Vitis clean TARGET=<t>` first).
