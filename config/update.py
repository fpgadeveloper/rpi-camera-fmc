...
def create_tables(data):
    ...
    for group in used_groups:
        ...
        tables.append('')
        tables.append('| Target board          | Target design   | FMC Slot | Cameras | VCU   | Baremetal<br> App | PetaLinux<br> Build | Yocto<br> Build | Vivado<br> Edition | IP<br>License |')
        tables.append('|-----------------------|-----------------|----------|---------|-------|-------|-------|-------|-------|-------|')
        for design in data['designs']:
            if not design['publish']:
                continue
...
def get_vitis_accel_targets(data):
    ...
    for design in data['designs']:
        if not design['accel']:
            continue
        target = '{}_target := 0 {}'.format(design['label'],design['cfg'])
        targets.append(target)
    return(targets)
...
# The Vivado IP Makefile is now a thin wrapper around build.sh and reads
# targets from data.json at runtime -- it no longer contains a generated
# target list.
